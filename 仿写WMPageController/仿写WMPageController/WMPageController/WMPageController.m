...
- (UIImage * _Nonnull)imageAtIndex:(NSInteger)index {
    UIImage * image = nil;
    if ([self.dataSource respondsToSelector:@selector(pageController:imageAtIndex:)]) {
        image = [self.dataSource pageController:self imageAtIndex:index];
    } else {
//        image = self.titles[index];
    }
    return image;
}
...
- (CGFloat)wm_calculateItemWithAtIndex:(NSInteger)index {
    ...
    return ceil(itemWidth + image.size.width);
...
-(UIImage *)menuView:(WMMenuView *)menu imageAtIndex:(NSInteger)index{
