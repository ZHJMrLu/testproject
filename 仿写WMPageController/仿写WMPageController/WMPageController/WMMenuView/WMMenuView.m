...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        item.image = [self.dataSource menuView:self imageAtIndex:i];
...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        if ([self.delegate respondsToSelector:@selector(menuView:widthForItemAtIndex:)]) {
            ...
            UIImage * image =  [self.dataSource menuView:self imageAtIndex:i];
            if (image) {
                NSInteger index = 0;
                if (self.selItem) {
                    index = self.selItem.tag - WMMENUITEM_TAG_OFFSET;
                }
                if (index == i) {
                    itemW = ceil(itemW + image.size.width);
                }
            }
        }
